...
class SyncScheduler:
    ...
    def _check_and_sync(self):
        try:
            ...
            for source in due_sources:
                (ds_id, connector_id, project_id, tenant_id, source_type,
                 ds_config, interval, last_sync, connector_config,
                 encrypted_config, connector_type) = source
                ...
                if encrypted_config:
                    ...
                    actual_config = connector_config if isinstance(
                        connector_config,
                        dict) else json.loads(connector_config)
                else:
                    actual_config = {}

                ds_cfg = ds_config if isinstance(ds_config, dict) else (
                    json.loads(ds_config) if ds_config else {})

                if connector_type == 'microsoft_teams' and not self._validate_teams_config(
                        actual_config, ds_id):
                    continue
                if connector_type == 'azure_devops' and not self._validate_devops_config(
                        actual_config, ds_id):
                    continue

                logger.info(
                    f"Auto-syncing data source {ds_id} (connector={connector_id}, type={connector_type}/{source_type})"
                )
                try:
                    if connector_type == 'microsoft_teams':
                        self._sync_teams_data_source(ds_id, connector_id,
                                                     project_id, tenant_id,
                                                     source_type, ds_cfg,
                                                     actual_config)
                    elif connector_type == 'azure_devops':
                        self._sync_devops_data_source(ds_id, connector_id,
                                                      project_id, tenant_id,
                                                      source_type, ds_cfg,
                                                      actual_config)
                    ...
                    self._record_failed_sync(tenant_id,
                                             project_id, connector_id, ds_id,
                                             str(e), connector_type,
                                             source_type)
    ...
    def _validate_teams_config(self, config: dict, ds_id: str) -> bool:
        ...
        if not all([client_id, client_secret, azure_tenant_id]):
            logger.warning(
                f"Connector for data source {ds_id} missing credentials, skipping"
            )
            return False

        if client_secret == "••••••••" or len(client_secret) < 8:
            logger.warning(
                f"Connector for data source {ds_id} has masked/invalid client_secret, skipping"
            )
            return False

        if not (UUID_PATTERN.match(azure_tenant_id)
                or DOMAIN_PATTERN.match(azure_tenant_id)):
            logger.warning(
                f"Connector for data source {ds_id} has invalid tenant_id '{azure_tenant_id}', skipping (must be a GUID or domain)"
            )
            return False

        if not UUID_PATTERN.match(client_id):
            logger.warning(
                f"Connector for data source {ds_id} has invalid client_id '{client_id}', skipping (must be a GUID)"
            )
            return False

        return True

    def _sync_teams_data_source(self, ds_id: str, connector_id: str,
                                project_id: str, tenant_id: str,
                                source_type: str, ds_config: dict,
        ...
        if source_type == "team_channel":
            ...
            if not team_id or not channel_id:
                logger.warning(
                    f"Data source {ds_id} missing team/channel config, skipping"
                )
                return
            ...
            messages = client.get_channel_messages(team_id,
                                                   channel_id,
                                                   since=since)

            self.vector_ops.insert_raw_messages(messages, "microsoft_teams",
                                                "team_channel", project_id,
                                                tenant_id, connector_id, ds_id)

            meeting_threads, chat_messages = build_meeting_threads(messages)

            openai_client = _make_openai_client()
            thread_engine = ThreadEngine(time_window_minutes=60,
                                         lookback_count=10,
                                         openai_client=openai_client)
            chat_threads = thread_engine.group_messages(chat_messages)

            processor = MessageProcessor(openai_client=openai_client,
                                         audio_processor=_audio_processor,
                                         teams_client=client)
            processed_meeting = [
                r for r in (processor.process_thread(t)
                            for t in meeting_threads) if r is not None
            ]
            processed_chat = [
                r for r in (processor.process_thread(t) for t in chat_threads)
                if r is not None
            ]

            meeting_added = self.vector_ops.add_threads(
                processed_meeting, "microsoft_teams", "meeting",
                source_identifier, project_id, tenant_id, connector_id, ds_id)
            chat_added = self.vector_ops.add_threads(
                processed_chat, "microsoft_teams", "team_channel",
                source_identifier, project_id, tenant_id, connector_id, ds_id)

            all_threads = meeting_threads + chat_threads
            for thread in all_threads:
                msg_ids = [
                    m["id"] for m in thread.get("messages", []) if m.get("id")
                ]
                if msg_ids:
            ...
            self._record_sync(tenant_id, project_id,
                              connector_id, ds_id, meeting_added + chat_added,
                              len(messages), "microsoft_teams", "team_channel")
            ...
            if not chat_id:
                logger.warning(
                    f"Data source {ds_id} missing chat config, skipping")
                return
            ...
            meeting_added = self.vector_ops.add_threads(
                ...
                source_identifier, project_id, tenant_id, connector_id, ds_id)
            chat_added = self.vector_ops.add_threads(
                processed_chat, "microsoft_teams", "group_chat",
                source_identifier, project_id, tenant_id, connector_id, ds_id)
            ...
            self._record_sync(tenant_id, project_id,
                              connector_id, ds_id, meeting_added + chat_added,
                              len(messages), "microsoft_teams", "group_chat")
    ...
    def _validate_devops_config(self, config: dict, ds_id: str) -> bool:
        ...
        if not organization:
            logger.warning(
                f"DevOps connector for data source {ds_id} missing organization, skipping"
            )
            return False
        ...
        if auth_type == "pat":
            ...
            if not pat or pat == "••••••••" or len(pat) < 8:
                logger.warning(
                    f"DevOps connector for data source {ds_id} has invalid PAT, skipping"
                )
                return False
        else:
            client_id = config.get("client_id", "")
            ...
            if not all([client_id, client_secret, azure_tenant_id]):
                logger.warning(
                    f"DevOps connector for data source {ds_id} missing Azure AD credentials, skipping"
                )
                return False
            if client_secret == "••••••••" or len(client_secret) < 8:
                logger.warning(
                    f"DevOps connector for data source {ds_id} has masked/invalid client_secret, skipping"
                )
                return False

        return True

    def _sync_devops_data_source(self, ds_id: str, connector_id: str,
                                 project_id: str, tenant_id: str,
                                 source_type: str, ds_config: dict,
                                 connector_config: dict):
        ...
        if auth_type == "pat":
            client = AzureDevOpsClient(organization=organization,
                                       auth_type="pat",
        ...
        if source_type == "devops_project":
            devops_project = ds_config.get("devops_project_name",
                                           ds_config.get("devops_project", ""))
            if not devops_project:
                logger.warning(
                    f"Data source {ds_id} missing devops_project_name config, skipping"
                )
                return
            ...
            messages = fetch_devops_work_items_as_messages(
                client, devops_project, since)

            added = self.vector_ops.add_messages(messages, "azure_devops",
                                                 "devops_project",
                                                 source_identifier, project_id,
                                                 tenant_id, connector_id,
                                                 ds_id)
            self._record_sync(tenant_id,
                              project_id, connector_id, ds_id, added,
                              len(messages), "azure_devops", "devops_project")
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (tenant_id, project_id, connector_id, ds_id, source_type,
                     segment_type, added, fetched),
                )
            conn.commit()
            conn.close()
        except Exception as e:
